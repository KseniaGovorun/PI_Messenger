...
namespace SimpleMessenger.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(LoginModel model)
        {
            ...
            {
                User user = await db.Users.FirstOrDefaultAsync(u => u.Email.Equals(model.Email) && u.Password.Equals(model.Password));
                System.Console.WriteLine("TETSTSTSTSSTSTSTSSTSTTSTSTSTTSTS");
                if (user!= null)
                {
                    await Authenticate(user); // авторизація
                    return RedirectToAction("Index", "Home");
                }
                ModelState.AddModelError("", "Неправильні логін і пароль");
            }
            return View(model);
        }
        [HttpGet]
